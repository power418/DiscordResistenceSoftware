...
// ---------------------------------------------------------------------------
// RecentActivityStore Implementations
// ---------------------------------------------------------------------------

std::string RecentActivityStore::normalize_key(std::string_view value) {
...
std::string RecentActivityStore::fallback_display_name(const RecentActivityEntry& entry) {
...
std::string RecentActivityStore::determine_display_name(
...
std::string RecentActivityStore::determine_details(const ActiveWindowInfo& snapshot,
                                                   const std::optional<ActivityPayload>& activity) {
...
std::string RecentActivityStore::determine_state(const ActiveWindowInfo& snapshot,
                                                 const std::optional<ActivityPayload>& activity,
...
std::string RecentActivityStore::fallback_display_name_from_snapshot(const ActiveWindowInfo& snapshot) {
...
std::string RecentActivityStore::build_subtitle(const RecentActivityEntry& entry) {
...
std::string RecentActivityStore::format_relative_time(std::uint64_t now,
                                                      std::uint64_t past) {
    if (now <= past) {
        return "just now";
    }
...
std::uint64_t RecentActivityStore::current_unix_timestamp_seconds() {
    return static_cast<std::uint64_t>(
        std::chrono::duration_cast<std::chrono::seconds>(
            std::chrono::system_clock::now().time_since_epoch()).count());
}

std::filesystem::path RecentActivityStore::cache_file_path() {
...
void RecentActivityStore::record(const ActiveWindowInfo& snapshot,
                                 const std::optional<ActivityPayload>& activity,
                                 bool new_visit) {
...
std::string RecentActivityStore::summary(std::size_t limit) const {
    ensure_file_cache_loaded();
...
void RecentActivityStore::save_file_cache() const {
...
void RecentActivityStore::ensure_file_cache_loaded() const {
...
void RecentActivityStore::prune_expired(std::uint64_t now) const {
...
std::string RecentActivityStore::history_key(const ActiveWindowInfo& snapshot) {
    if (snapshot.exe_path.empty()) {
        return {};
    }
    return normalize_key(snapshot.exe_path);
}

std::uint64_t RecentActivityStore::cache_ttl_seconds() {
    return 30ULL * 24ULL * 60ULL * 60ULL; // 30 days
}

std::uint64_t RecentActivityStore::persist_interval_seconds() {
    return 60ULL * 5ULL; // 5 minutes
}
